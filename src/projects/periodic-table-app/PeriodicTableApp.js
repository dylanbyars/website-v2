...
class PeriodicTableApp extends Component {
  ...
  componentDidMount() {
    axios.get('https://rawgit.com/dbyars/periodic-table-1/master/data.json').then(({ data }) => {
      const groups = [
        [1, 3, 11, 19, 37, 55, 87],
        [4, 12, 20, 38, 56, 88],
        [21, 39],
  ...
  render() {
    return !this.state.elements.length ? (
      <div className="min-h-screen flex justify-around items-center">
        <GridLoader loading={!this.state.elements.length} size={60} margin="20px" color="#6574cd" />
      </div>
    ) : (
      <div
      ...
      >
        ...
        <div className={classnames(['flex', 'flex-col', { blur: this.state.activeElement }])}>
          <div className="flex pb-6">
            {this.state.groups.map(
              (elements, i) =>
                i !== 2 ? (
                  <div key={elements[0].atomicMass} className="flex flex-col justify-end">
                    {elements.map(element => (
                      <ElementBlock key={element.name} element={element} setActiveElement={this.setActiveElement} />
                    ))}
                  </div>
                ) : (
                  <div key={elements[0].atomicMass} className="flex flex-col justify-end">
        ...
        {this.state.activeElement && <Modal element={this.state.activeElement} closeModal={this.clearActiveElement} />}
      </div>
    )
  }
}
